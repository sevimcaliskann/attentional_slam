...
#include "VOCUS2_cuda_ext.h"
using namespace std;

VOCUS2_cuda_ext::VOCUS2_cuda_ext(){
...
VOCUS2_cuda_ext::VOCUS2_cuda_ext(const VOCUS2_Cfg& cfg) {
	// use given config
	this->cfg = cfg;
...
void VOCUS2_cuda_ext::setCfg(const VOCUS2_Cfg& cfg) {
  this->cfg = cfg;
  this->salmap_ready = false;
  this->splitted_ready = false;
}

void VOCUS2_cuda_ext::write_out(string dir){
...
void VOCUS2_cuda_ext::process(const Mat& img){
...
void VOCUS2_cuda_ext::gaborFilterImages(const Mat &base, Mat &out, int orientation, float sigma, int scale){
...
void VOCUS2_cuda_ext::pyramid_codi(const Mat& img){
...
void VOCUS2_cuda_ext::pyramid_new(const Mat& img){
	// clear previous results
	clear();
...
void VOCUS2_cuda_ext::pyramid_classic(const Mat& img){
	// clear previous results
	clear();
...
vector<Mat> VOCUS2_cuda_ext::build_multiscale_pyr(Mat& mat, float sigma){
...
void VOCUS2_cuda_ext::center_surround_diff(){
    int on_off_size = 3*2;
...
std::vector<cv::Mat> VOCUS2_cuda_ext::getFeatureChannel(FeatureChannels name){
    switch(name){
    case ON_OFF_L:
        return on_off_L;
    case OFF_ON_L:
...
void VOCUS2_cuda_ext::orientationWithCenterSurroundDiff(){
...
Mat VOCUS2_cuda_ext::get_salmap(){
...
Mat VOCUS2_cuda_ext::add_center_bias(float lambda){
...
vector<Mat> VOCUS2_cuda_ext::get_splitted_salmap(){
	if(!processed){
		cout << "Image not yet processed. Call process(Mat)." << endl;
...
float VOCUS2_cuda_ext::compute_uniqueness_weight(Mat& img, float t = 0.5){
...
Mat VOCUS2_cuda_ext::fuse(vector<Mat> maps, FusionOperation op){
...
vector<Mat> VOCUS2_cuda_ext::prepare_input(const Mat& img){
